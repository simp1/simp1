<?php
	...
	if(checktoken($token,$token_login,$username)){
		...
		if($erg>=0){
			...
			$output .= "<tr><th>WerkzeugID</th><th>GeoID</th><th>Kavitaet</th><th>Position</th><th>auswerfer</th>";
			$output .= "</tr></thead>";
			while($ausgabe = $statemt->fetch_object()){
			...
			$output .= "<div class='table-responsive'><table class='table table-striped'>";
			$output .= "<thead class='thead-dark'>";
			$output .= "<tr><th>Bohrbild</th><th>Form</th><th>Aussenmass</th><th>MassAs</th><th>MassDs</th>";
			$output .= "</tr></thead>";
			$sql="SELECT * FROM werkzeuggeometrie WHERE werkzeugID='".$werkzeugID."'";
			$statemt = getsql($sql);
			while($ausgabe = $statemt->fetch_object()){
			...
			$output .= "<tr><td>".$bohrbild."</td><td>".$form."</td><td>".$ausmas."</td><td>".$massas."</td><td>".$massds."</td>";
			$output .="</table></div>";	
			...
			$output .= "<div class='table-responsive'><table class='table table-striped'>";
			$output .= "<thead class='thead-dark'>";
			$output .= "<tr><th>Bezeichnung</th><th>Wert</th>";
			$output .= "</tr></thead>";
			while($ausgabe = $statement->fetch_object()){
				$bezeichnung = $ausgabe->bezeichnung;
				$wert = $ausgabe->wert;
				$output .= "<tr><td>".$bezeichnung."</td><td>".$wert."</td></tr>";
			}
			$output .="</tr></table></div>";	
