		...
		if($erg>=0){
			...
			while($ausgabe = $statemt->fetch_object()){
				...
				while($ausgabe = $statemt->fetch_object()){
					...
					if($erg>=1){
					  	$output .="<td><button type='button' class='butosuccess' id=".$id_attr." onClick='entfernen(id)'>entfernen</button></tr></td>";//
					}else{
						$output .="<td><button type='button' class='butosuccess' onClick='norights(id)'>entfernen</button></tr></td>";
					}
				}
				$output .="</table></div>";
